...
QueryFrame SqLiteBuilder::makeKeywordFrame(const QueryClause &queryClause) const
{
	return AbstractQueryBuilder::makeKeywordFrame(queryClause);
}

/*! Компонует один фрейм для одного элемента секции. */
QueryFrame SqLiteBuilder::makeClauseItemFrame(const QueryBatch &batch,
...
QueryBatchList SqLiteBuilder::validClauseBatches(const QueryClause &queryClause,
...
QueryFrame SqLiteBuilder::_conditionClauseFrame(const QueryWhere &whereCondition,
