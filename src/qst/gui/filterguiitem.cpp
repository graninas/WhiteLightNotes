...
/*!
\lang_Russian
	Возвращает QueryValue: QVariant-value и функтор сравнения.
	Если виджет - QComoBox с привязанным хэндлером, то возвращает
	id текущей записи.
\endlang

\lang_English
	Returns QueryValue: vaule entered in the widget (QVariant) and comparison functor.
	If the widget - QComoBox binded with the handler, then returns
...
QueryField FilterGuiItem::queryField() const
{
	...
	{
	case Filter::LineEdit:
	case Filter::DateEdit:	return QueryField("", QueryValue(value()), _filter.functor());
	case Filter::ComboBox:
		...
		if (_bindedHandler == NULL)
			...
			return QueryField("", QueryValue(_bindedHandler->comboBoxKeyValue()), _filter.functor());
	}
return QueryField();
}
...
void FilterGuiItem::_detachHandlerView()
{
	...
	case Filter::ComboBox:
		...
		QComboBox *comboBox = (QComboBox *) _inputWidget;
		_bindedHandler->detachComboBox(comboBox);
	}
}
